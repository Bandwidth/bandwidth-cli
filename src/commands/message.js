...
module.exports.messageAction = async (toNum, cmdObj) => {
  ...
  BandwidthMessaging.Configuration.basicAuthPassword = creds.password; // The password to use with basic authentication
  const accId = await utils.readAccountId();
  const opts = cmdObj.opts();
  const fromNum = await utils.processDefault('number', opts.fromNum);
  const application = await utils.processDefault('application', opts.application);
  if (!fromNum) {
  ...
  printer.success('The following information passed to server:');
  printer.printObj(response)
  printer.warn('Warning: The message is not necessarily delivered. Callback information about the text should be accessed via a server. Set up a server with for your default application with "bandwidth code server".')
}
